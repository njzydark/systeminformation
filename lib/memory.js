      ...
      if (_darwin) {
        ...
        try {
          let formatedPagesize = Number(
            execSync("sysctl vm.pagesize").toString().split(":")[1].trim()
          );
          if (formatedPagesize) {
            pagesize = formatedPagesize;
          }
        } catch (error) {}
            ...
            if (!error) {
              ...
              if (lines.length > 0) {
                ...
                for (let i = 0; i < line.length; i++) {
                  if (line[i].toLowerCase().indexOf('total') !== -1) { result.swaptotal = parseFloat(line[i].split('=')[1].trim()) * pagesize; }
                  if (line[i].toLowerCase().indexOf('used') !== -1) { result.swapused = parseFloat(line[i].split('=')[1].trim()) * pagesize; }
                  if (line[i].toLowerCase().indexOf('free') !== -1) { result.swapfree = parseFloat(line[i].split('=')[1].trim()) * pagesize; }
                }
              }
            }
            if (callback) { callback(result); }
